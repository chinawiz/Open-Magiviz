...
## ~~进行中的专项:operate.tsx 拆分~~(已完成 2026-09-04)

T1-T18+批次6(T19 死代码终清/T21a mapper 合一/T21b 接线收敛)全部完工:operate.tsx 10,037→**1,693 行**(编排+装配层,0 warning),棘轮 430→300,183 tests。完工线 <1,500 未达(1,693),差距为 T20(state 合并)——经裁决放弃(依赖循环+收益/风险倒挂,详见 plan.md 收官节)。遗留:剧情视频真实供应商段补验(随部署批次)、分镜段 data.scenes 直访共性遗留、T6/T7 hook 存量 any。
