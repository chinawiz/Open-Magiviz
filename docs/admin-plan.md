...
- 状态：**P0 + P1 + P2 全部上线（b8ef6e0/a49d93c/17045fb）并通过运营者真实 GUI 验收（2026-08-30 六页全过）**；P3（newsletter 重写看订阅规模 / Kie 对账留脚本）待定
- 日期：2026-08-28 初稿 / 2026-08-30 评审修订 / 2026-08-30 P0+P1 落地并上线 / 2026-08-30 P2 落地
- 部署前置：无（0015 已在生产执行、管理员已引导；P2 无新迁移）
- P2 实施备注：growth 提现审批 UI 经盘点**已存在**（表格+处理对话框+校验），实际缺口是 PATCH 审计与双确认——已补；退款单可见依赖 webhook 补 `charge.refunded` 回写 stripePayments（幂等、fail-safe，积分回收仍走 finance 页手动调减）——已补；勾稽口径「应发」含已退款单（succeeded+refunded）
