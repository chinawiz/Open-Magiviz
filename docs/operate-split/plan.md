...
风险升序:纯数据 → 展示 → 状态。T1/T2 无阻塞;T3/T4 无相互依赖;T5-T8 依赖 T1(状态块要引用新注册表)且 T6/T8 有相互依赖(见各自 ticket 的 blocked-by)。**钱权安全网已就位**:points/points-manager/webhook-security/payments 四模块单测已补(2026-09-03,150 tests)。
...
- [ ] T3 剩余 Dialog → components/operate/(批次二起点,验收需精简 GUI 回归)
- [ ] T4 积分/计费/订阅展示段
- [ ] T5 素材库选择状态块
- [ ] T6 故事板生成状态块
- [ ] T7 图片生成状态块
