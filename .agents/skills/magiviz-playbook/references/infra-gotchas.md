...
29. **Next 16 起仓库的 lint 是「双重坏死」的，接 lint 前先查两条版本线（2026-09-03 接入实证）**：① `next lint` 命令在 Next 16 已被移除，老 script `"lint": "next lint"` 必须改成直接调 `eslint .`；② **ESLint 别装最新大版本**——`eslint-plugin-react`（eslint-config-next 传递依赖，7.37.5）peer 只到 `^9.7`，装 ESLint 10 会在 `react/display-name` 等规则上直接 `TypeError: contextOrFilename.getFilename is not a function` 崩掉，锁 `eslint@^9` 即愈。好消息：eslint-config-next 16 导出原生 flat config 数组，`eslint.config.mjs` 直接 `import ... from "eslint-config-next/core-web-vitals"` 展开即可，**不再需要 FlatCompat**。ignores 别忘了 `.open-next/**`（带点前缀，只写 `open-next/**` 拦不住，构建产物被扫会先炸出插件兼容错）。锚点：`eslint.config.mjs`、`package.json` scripts、`QUALITY-CHECKLIST.md`「Lint 规则口径」节。

30. **i18n/SEO/滚动三缺陷同源根因（2026-09-04 生产冒烟实证与修复）**：① `<html lang>` 由 root layout 按 `x-next-intl-locale` 请求头在服务端首载时输出一次（见 #20），客户端切换语言是路由内跳转、不会重渲染 root layout——lang 停留旧语言（/zh 页 lang="en"）。修法：`[locale]/layout` 挂 `HtmlLangSync` 客户端组件按 locale 同步 `document.documentElement.lang`。② metadata 标题模板会与子页 title 复合：子页自带「 - MeiHao」后缀再叠加模板 `%s | ${t('title')}` 产出「价格 - MeiHao | MeiHao - 创意…」双品牌。口径固定为**模板管品牌、子页 title 用裸节名**（messages 里 `metadata.*.title`/`auth.meta_title` 等 18 处已剥后缀）。③ App Router 部分客户端导航不回页顶（页脚法律链接实测落在新页中部、sticky 导航遮章节标题），且 globals.css 的 `scroll-behavior: smooth` 会把 `scrollTo(0,0)` 变成逐帧回滚动画——`ScrollToTopOnNavigate` 兜底必须 `behavior: 'instant'` 绕过 CSS。两个非视觉副作用组件集中在 `components/route-effects.tsx` 挂 `[locale]/layout.tsx`，未来加语言/动路由先想起它们。

30. **「影子文件」是改一漏一事故的完美温床（2026-09-03 实证）**：`components/ui/use-toast.ts` 与 `hooks/use-toast.ts` 194 行**完全相同**，且各自被不同组件真实 import（4:1 分流）——改任何一份，另一半静默保持旧行为，tsc/测试/build 全绿。发现手法：`diff` 两文件为零 + `grep -rln` 双向查 import 方；处置：保留多数派路径（hooks/），改 1 处 import 后删除影子，再全库 grep 确认零残留。通用规律：**发现两个同名同长度文件，第一反应先 diff，而不是假设只是巧合**。

31. **neon() 驱动拒绝 `postgres://` 短格式连接串（2026-09-04 实证）**：`@neondatabase/serverless` 的 neon() 校验 URL 必须 `postgresql://user:pass@host/db` 全格式（`postgres://localhost/db` 直接抛「Database connection string format」）。测试里为了放行 `lib/db` 顶层加载设哑 env 时要用全格式（不真连，只过构造）；更稳的路线是 `vi.hoisted(() => { process.env.DATABASE_URL ??= 'postgresql://...' })` 抢在静态 import 之前。锚点：`lib/db.ts`、`lib/providers/endpoints.test.ts`。

32. **vitest include 是白名单，路由级测试默认不收（2026-09-04 实证）**：原配置只收 `lib/components/hooks` 三目录，写在 `app/api/**/route.test.ts` 的测试会「No test files found」静默溜走——首个路由级契约测试落地时把 `app/**/*.test.ts` 加进 `vitest.config.ts` include。next-intl messages 大段插键别用文本锚点硬插：真实事故是手工锚点把新命名空间插进了隔壁父级（admin.models 误入 admin.finance，语义上还编译得过）——先验证 `json.dumps(data, ensure_ascii=False, indent=2)+'\n'` 与原文件逐字节一致，再程序化插入，最后双语键集合差集断言（插错层级/缺键全被断言网住）。锚点：`vitest.config.ts`、`app/api/admin/models/route.test.ts`、`messages/*.json`。
